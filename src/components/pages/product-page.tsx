...
export function ProductPage({
  ...
  return (
    <>
      <PageHero eyebrow={product.zone} title={product.title} copy={product.type} image={product.heroImage} meta={[product.zone, product.type, dict.product.madeToOrder]} />
      ...
      <section className="grid grid-cols-[minmax(0,0.9fr)_minmax(300px,0.65fr)] items-center gap-[8%] px-[8%] py-[8%] max-lg:grid-cols-1 max-md:px-[4%]">
        <div className="max-w-3xl">
          <span className="mb-4 block text-[11px] font-bold uppercase tracking-[0.28em] text-[#8f6747]">{dict.product.profileEyebrow}</span>
          ...
          <div className="mt-8 grid grid-cols-3 gap-px bg-[#d8cec3] max-md:grid-cols-1">
            {[dict.product.naturalStone, product.zone, dict.product.outdoorWellness].map((item) => (
        ...
        <section className="bg-[#e8ece6] px-[8%] py-[8%] max-md:px-[4%]">
          <div className="mb-12 grid gap-8 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
            <div className="max-w-4xl">
              <span className="mb-4 block text-[11px] font-bold uppercase tracking-[0.28em] text-[#8f6747]">{dict.product.configurationEyebrow}</span>
              <h2 className="font-heading text-[clamp(2.6rem,6vw,6.4rem)] font-normal leading-[0.88] tracking-normal text-[#151411]">{dict.product.materialsTitle}</h2>
              <p className="mt-6 max-w-2xl text-base leading-8 text-[#5b554f] md:text-lg">
                {dict.product.materialsCopy}
              </p>
            </div>
      ...
      <QuoteForm locale={locale} product={product} />

      {pairs.length ? (
        <section className="px-[8%] py-[8%] max-md:px-[4%] max-md:pt-14">
          <SectionHeading title={dict.product.pairWith.replace("{product}", product.title)} />
      ...
      {product.designedBy ? <DesignerSection locale={locale} /> : null}
    </>
  );
}
